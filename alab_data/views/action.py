...
class ActionView:
    ...
    def add(self, action: Action) -> ObjectId:
        # TODO type check material. maybe this is done within Material class idk
        if action.in_database:
            raise ValueError(
                f"Action {action.name} already exists in the database!"
            )
    ...
    def get(self, id: ObjectId) -> Action:
        data = self._collection.find_one({"_id": id})
        if data is None:
            raise ValueError(f"Cannot find an Action with id: {id}")
        action = Action(**data)
        action.id = id
        return action
